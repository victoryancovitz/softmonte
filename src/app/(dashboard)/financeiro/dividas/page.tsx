...
export default async function DividasPage() {
  ...
  const [{ data: dividas }, { data: indicadores }, { data: contas }, { data: fornecedores }, { data: centros }, { data: credorTipos }] = await Promise.all([
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      ...
      <DividasClient dividas={dividas ?? []} indicadores={indicadores} contas={contas ?? []} fornecedores={fornecedores ?? []} centros={centros ?? []} credorTipos={credorTipos ?? []} />
    </div>
  )
}
