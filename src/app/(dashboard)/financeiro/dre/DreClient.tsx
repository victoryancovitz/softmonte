...
export default function DreClient({ dre, dreMes, custos, lancamentos }: { dre: any[]; dreMes: any[]; custos: any[]; lancamentos: any[] }) {
  const [expandido, setExpandido] = useState<string | null>(null)
  const [tab, setTab] = useState<'margem' | 'dre'>('margem')

  // DRE consolidado por mês
  ...
  return (
    <>
      ...
      <div className="flex gap-1 mb-5 bg-gray-100 p-1 rounded-lg w-fit">
        {([
          { key: 'margem' as const, label: 'Margem por Contrato' },
                  ...
                  {isOpen && (
                    <div className="border-t border-gray-100 bg-gray-50/50">
                      {(() => {
                        ...
                        if (mesesObra.length === 0) return <div className="px-5 py-4 text-sm text-gray-400">Sem dados mensais para esta obra.</div>
                        ...
                        return (
                          <div className="px-5 py-3 border-b border-gray-100">
                            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">DRE Real Mês a Mês</h3>
                              ...
                              <table className="w-full text-sm">
                                  ...
                                  {mesesObra.map((m: any) => {
                                    ...
                                    return (
                                      <tr key={`${m.ano}-${m.mes}`} className="border-b border-gray-100">
                                        ...
                                        <td className="px-3 py-2 text-gray-500">{Number(m.dias_descontados) > 0 ? <span className="text-red-600 font-medium">{Number(m.dias_descontados).toFixed(1)}d</span> : '—'}</td>
                                        ...
                                        <td className={`px-3 py-2 font-bold ${margem >= 0 ? 'text-green-700' : 'text-red-700'}`}>{fmt(margem)}</td>
                                        <td className={`px-3 py-2 font-bold ${margemPct >= 30 ? 'text-green-700' : margemPct >= 15 ? 'text-amber-700' : 'text-red-700'}`}>{margemPct.toFixed(1)}%</td>
                                      </tr>
                                    )
                                  })}
                                </tbody>
                                <tfoot>
                                  <tr className="bg-brand/5 border-t-2 border-brand/20 font-bold">
                                    ...
                                    <td className={`px-3 py-2 ${totMargemPct >= 30 ? 'text-green-700' : totMargemPct >= 15 ? 'text-amber-700' : 'text-red-700'}`}>{totMargemPct.toFixed(1)}%</td>
                                  </tr>
                                </tfoot>
                              </table>
                            </div>
                        ...
                        {funcsObra.length > 0 ? (
                          <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                              <thead>
                                <tr className="border-b border-gray-200">
                                  {['Funcionário', 'Salário Base', 'Custo Total/Mês', 'Custo/Hora', 'Billing Rate', 'Margem/HH', 'Margem %'].map(h => (
                                    <th key={h} className="text-left px-3 py-2 text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">{h}</th>
                                  ))}
                                </tr>
                              </thead>
                              <tbody>
                                {funcsObra.map((f: any) => (
                                  <tr key={f.funcionario_id} className="border-b border-gray-100">
                                    <td className="px-3 py-2"><div className="font-medium text-gray-900">{f.nome}</div><div className="text-xs text-gray-400">{f.funcao_no_contrato}</div></td>
                                    ...
                                    <td className={`px-3 py-2 font-semibold ${Number(f.margem_hh) >= 0 ? 'text-green-700' : 'text-red-700'}`}>{fmt(f.margem_hh)}/h</td>
                                    <td className={`px-3 py-2 font-bold ${Number(f.margem_pct) >= 0 ? 'text-green-700' : 'text-red-700'}`}>{Number(f.margem_pct).toFixed(1)}%</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
                          </div>
                        ) : <p className="text-sm text-gray-400">Sem dados de custo para esta obra.</p>}
                      </div>
                    </div>
                  )}
                </div>
              ...
              {dreMeses.length > 0 ? dreMeses.map(m => {
                ...
                return (
                  <tr key={m.mes} className="border-b border-gray-50 hover:bg-gray-50/80">
                    ...
                    <td className={`px-4 py-2.5 text-right font-bold ${resultado >= 0 ? 'text-green-700' : 'text-red-700'}`}>{fmt(resultado)}</td>
                    <td className={`px-4 py-2.5 text-right font-bold ${margemPct >= 30 ? 'text-green-700' : margemPct >= 15 ? 'text-amber-700' : 'text-red-700'}`}>{margemPct.toFixed(1)}%</td>
                  </tr>
                )
              }) : (
                <tr><td colSpan={7} className="px-4 py-10 text-center text-gray-400">
                  <TrendingUp className="w-10 h-10 text-gray-300 mx-auto mb-3" />
                  <p>Sem lançamentos financeiros para gerar DRE.</p>
                </td></tr>
              )}
            </tbody>
            {dreMeses.length > 0 && (
