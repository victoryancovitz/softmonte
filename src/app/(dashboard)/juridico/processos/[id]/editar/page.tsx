...
export default function EditarProcessoPage() {
  ...
  const [form, setForm] = useState<any>({
    numero_cnj: '', tipo: '', status: '', objeto: '', polo: '', parte_contraria: '',
  ...
  async function loadData() {
    const [{ data: p }, { data: funcs }, { data: obr }, { data: cc }, { data: advs }] = await Promise.all([
      supabase.from('processos_juridicos').select('*').eq('id', id).single(),
    ...
    if (p) {
      setForm({
        ...
        parte_contraria_cpf_cnpj: p.parte_contraria_cpf_cnpj || '',
        url_processo: p.url_processo || '',
        tribunal: p.tribunal || '',
        vara: p.vara || '',
        comarca: p.comarca || '',
        uf: p.uf || '',
        ...
        valor_causa: p.valor_causa?.toString() || '',
        valor_provisionado: p.valor_provisionado?.toString() || '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.from('processos_juridicos').update(payload).eq('id', id)
    setLoading(false)

    if (err) { setError(err.message); return }

    toast.success('Processo atualizado com sucesso')
    router.push(`/juridico/processos/${id}`)
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-xl font-bold font-display text-brand mt-4 mb-6">Editar Processo</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <fieldset className="border rounded-xl p-4 space-y-4">
          <legend className="text-sm font-semibold text-gray-700 px-2">Dados Básicos</legend>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nº CNJ</label>
              <input type="text" value={form.numero_cnj} onChange={e => set('numero_cnj', e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm font-mono" />
            </div>
            <div>
          ...
          <div>
            ...
            <input type="text" value={form.url_processo} onChange={e => set('url_processo', e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm" />
          </div>
        </fieldset>

        {/* Tribunal */}
        <fieldset className="border rounded-xl p-4 space-y-4">
          <legend className="text-sm font-semibold text-gray-700 px-2">Tribunal</legend>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Tribunal</label>
              <input type="text" value={form.tribunal} onChange={e => set('tribunal', e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
        ...
        {/* Financeiro */}
        <fieldset className="border rounded-xl p-4 space-y-4">
          <legend className="text-sm font-semibold text-gray-700 px-2">Financeiro e Vínculos</legend>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
